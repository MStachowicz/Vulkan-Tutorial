...
The `glfwInit()` call initializes the GLFW library. On macOS, provide GLFW with
Vulkan's loader before initializing it:
...
Because GLFW was originally designed to create an OpenGL context, we need to
tell it to not create an OpenGL context with a later call:
